#!/usr/bin/env bash
#
# Version consistency gate for the KnishIO Kotlin SDK.
#
...
collect "build.gradle.kts version" \
  "$(sed -n 's/^version *= *"\([^"]*\)".*/\1/p' build.gradle.kts | head -1)"
# SelfTest.kt reports this into kotlin-results.json; the gauntlet's coherence gate compares it
