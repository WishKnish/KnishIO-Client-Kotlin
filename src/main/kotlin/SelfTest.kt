...
class KotlinSelfTest {
     ...
     * molecular hash as the JS reference. Call AFTER all atoms are added, BEFORE sign().
     */
    private fun setFixedTimestamps(molecule: Molecule) {
        val base = 1700000000000L
        molecule.atoms.forEachIndexed { i, atom ->
            atom.createdAt = (base + i * 1000L).toString()
        }
    }
    ...
    fun testMetaCreation(config: JsonObject): Boolean {
        ...
        return try {
            ...
            // Canonical fixed remainder wallet (mirrors JS createFixedRemainderWallet); token 'USER'
            // keeps the ContinuID flow on the canonical bbbb... remainder deterministically.
            val remainderWallet = Wallet(secret, token, "bbbb000000000000cccc111111111111dddd222222222222eeee333333333333")

            // Create molecule instance
            val molecule = Molecule(secret, sourceWallet, remainderWallet)
    ...
    fun testSimpleTransfer(config: JsonObject): Boolean {
        ...
        return try {
            ...
            // Canonical fixed remainder wallet (mirrors JS); initValue's remainder atom lands at bbbb...
            val remainderWallet = Wallet(sourceSecret, token, "bbbb000000000000cccc111111111111dddd222222222222eeee333333333333")
    ...
    fun testComplexTransfer(config: JsonObject): Boolean {
        ...
        return try {
            ...
            val remainderPosition = "bbbb000000000000cccc111111111111dddd222222222222eeee333333333333"
            val remainderWallet = Wallet(sourceSecret, token, remainderPosition)
            logTest("Remainder wallet creation", true)
            
            // Create recipient wallet
            val recipient1Secret = Crypto.generateSecret(recipient1Seed, 2048)
                    ...
                    for ((moleculeType, moleculeData) in molecules.entrySet()) {
                        try {
                            if (moleculeType == "mlkem768") {
                                ...
                                if (config != null) {
                                    val testConfig = config.getAsJsonObject("tests").getAsJsonObject("mlkem768")
                                    val secret = Crypto.generateSecret(testConfig.get("seed").asString, 2048)
