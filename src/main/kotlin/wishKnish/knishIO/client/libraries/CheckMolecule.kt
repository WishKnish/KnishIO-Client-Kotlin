...
class CheckMolecule {
    companion object {
        ...
        fun enumerate(hash: String): Map<Int, Int> {
            val mapped = mapOf('0' to -8, '1' to -7, '2' to -6, '3' to -5, '4' to -4, '5' to -3, '6' to -2,
                '7' to -1, '8' to 0, '9' to 1, 'a' to 2, 'b' to 3, 'c' to 4, 'd' to 5, 'e' to 6, 'f' to 7, 'g' to 8)
        ...
        fun isotopeR(molecule: Molecule): Boolean {
            ...
            molecule.atoms.filter { it.isotope == 'R' }.forEach {
                setOf("callback", "conditions", "rule").forEach { key ->
                    val metaData = it.meta.find { metaData -> metaData.key == key }
                        ?: throw MetaMissingException("Check::isotopeR() - Required meta field \"${key}\" is missing!")
        ...
        fun ots(molecule: Molecule): Boolean {
            ...
            var ots = molecule.atoms.map { it.otsFragment }
